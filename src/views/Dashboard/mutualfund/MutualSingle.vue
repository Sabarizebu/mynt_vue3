      ...
      <div class="body-part">
        <div class="mb-6 pos-rlt default-tab">
          <v-toolbar class="d-none d-md-block px-0 pos-stk z-i3" style="top: 0px" flat density="compact" color="cardbg">
            <v-tabs @update:model-value="setbodyTab" fixed color="maintext" v-model="bodytab" show-arrows density="compact">
              <v-tab density="compact" v-for="(t, index) in dashitems" :key="index" class="text-none text-start min-w-fit">
                <span class="text-center tab-txt">{{ t.txt }}</span></v-tab
              >
              <v-tabs-slider color="maintext"></v-tabs-slider>
            </v-tabs>
            <v-spacer></v-spacer>
            <v-btn v-if="uid && menudata.Purchase_Allowed == 'Y'" @click="putMForder('buy')" size="small" class="text-none font-weight-bold elevation-0 rounded-pill mr-2 maingreen--text fs-14" color="secgreen">Buy</v-btn>
            <v-btn v-if="uid && menudata && menudata.params && menudata.params.SIP_FLAG == 'Y'" @click="putMForder('sip')" size="small" class="text-none font-weight-bold elevation-0 rounded-pill mr-2 primary--text fs-14" color="secbg">SIP</v-btn>
          </v-toolbar>
          ...
          <div>
              ...
              <v-card color="secbg" class="elevation-0 rounded-0 pt-2 px-md-2 px-3">
                <v-row no-gutters>
                  <v-col cols="12" md="8" class="pb-1 pb-md-2">
                    <v-list-item class="px-0 px-md-4">
                      <template v-slot:prepend>
                        <v-avatar color="#fbfbfb" size="56" class="my-0 mr-4">
                      ...
                      <v-chip-group class="my-0 py-0">
                        <v-chip v-if="menudata.params && menudata.params.splito" label size="x-small" text-color="subtext" color="cardbg" class="font-weight-medium fs-10">
                          {{ menudata.params.splito }}
                        </v-chip>
                        <v-chip v-if="menudata.params && menudata.params.splitr" label size="x-small" text-color="subtext" color="cardbg" class="font-weight-medium fs-10">
                          {{ menudata.params.splitr }}
                        </v-chip>
                        <v-chip v-if="menudata.params && menudata.params.splitt" label size="x-small" text-color="subtext" color="cardbg" class="font-weight-medium fs-10">
                  ...
                  <v-col cols="12" md="4" class="text-md-right text-left pt-0 pt-md-5 pb-2 pl-4 pl-md-3">
                    <v-list-item class="px-0 px-md-4">
                      <v-list-item-title class="fs-14 maintext--text font-weight-bold">
                        ...
                        >
                        {{ Number(menudata.currentNAV) ? `₹ ${Number(menudata.currentNAV).toFixed(2)}` : "0.00" }}
                      </v-list-item-title>
                      <v-list-item-title class="fs-14 maintext--text font-weight-bold">
                        <v-list-item-subtitle class="fs-12 subtext--text text-uppercase">5yr CAGR</v-list-item-subtitle>
                        <span :class="menudata['5Year'] > 0 ? 'maingreen--text' : menudata['5Year'] < 0 ? 'mainred--text' : 'subtext--text'"
                          ><span :class="!uid ? 'blur-text' : ''">{{ menudata['5Year'] ? `${Number(menudata['5Year']).toFixed(2)}%` : "0.00" }}</span></span
                        >
                      </v-list-item-title>
                      <v-list-item-title class="fs-14 maintext--text font-weight-bold">
                        <v-list-item-subtitle class="fs-12 subtext--text text-uppercase">AMU (Cr.)</v-list-item-subtitle>
                        {{ menudata.params ? `${(Number(menudata.params.AUM) / 10000000).toFixed(2)}` : "0.00" }}
                      </v-list-item-title>
                    </v-list-item>
                  </v-col>
                </v-row>
              ...
              <div class="px-md-6 px-4 pt-4 pt-md-8">
                <v-row no-gutters>
                  <v-col cols="12" md="10">
                    ...
                    <p class="fs-14 subtext--text txt-x-j mb-0">{{ menudata.overview ? menudata.overview.split("The fund portfolio")[0] : "-" }}</p>
                  </v-col>

                  <v-col cols="12" md="2" class="pt-5">
                    <img v-if="menudata && menudata.risk" :src="getRiskMeterImage(menudata.risk)" alt="riskmeter" width="100%" />
                  </v-col>
                </v-row>
                  ...
                  <v-col cols="12" md="12">
                    ...
                    <v-row no-gutters>
                      <v-col cols="4" class="pb-0" v-for="(t, d, l) in volatilityfield" :key="l">
                        <v-text-field class="funda-field mb-0" readonly color="maintext" :label="d" :model-value="t ? t : '-'"> </v-text-field>
                      </v-col>
                    </v-row>
                  </v-col>
                </v-row>
            ...
            <v-card id="info" class="crd-trn ss-cards overflow-hidden mb-md-6" width="100%">
              <div class="pt-md-6 py-4 px-4 px-md-6">
                ...
                <v-row no-gutters>
                  <v-col cols="12" md="7">
                    ...
                    <p class="fs-14 mb-0 txt-x-j">{{ menudata.overview ? `The fund portfolio ${menudata.overview.split("The fund portfolio")[1].split("Additional details")[0]}` : "-" }}</p>
                  </v-col>
                  <v-col cols="12" md="5">
                    ...
                    <v-card variant="outlined" class="rounded-lg mb-4">
                      <v-list-item class="px-0 px-md-3">
                        <template v-slot:prepend>
                          <v-avatar color="#fbfbfb" size="52" class="mb-auto mt-3 mr-3">
              ...
              <v-row class="px-4 px-md-6 mb-md-3 mb-4">
                <v-col cols="4" class="pb-0" v-for="(t, d, l) in schemefield" :key="l">
                  <v-text-field class="funda-field" readonly color="maintext" :label="d" :model-value="t ? t : '-'"> </v-text-field>
                </v-col>
              </v-row>
            </v-card>
                  ...
                  <v-col cols="12" md="4">
                    ...
                    <div v-for="(s, j) in menudata && menudata.sectors ? menudata.sectors.slice(0, 10) : 10" :key="j">
                      <div class="mb-5">
                        ...
                        <v-progress-linear v-if="s.netAsset" color="#148564" height="6px" rounded :model-value="Math.round(s.netAsset)"></v-progress-linear>
                      </div>
                    </div>
                  </v-col>
                  <v-col cols="12" md="4">
                    ...
                    <div v-for="(s, j) in menudata && menudata.holdings ? menudata.holdings.slice(0, 10) : 10" :key="j">
                      <div class="mb-5">
                        ...
                        <v-progress-linear v-if="s.netAsset" color="#7CD36E" height="6px" rounded :model-value="Math.round(s.netAsset)"></v-progress-linear>
                      </div>
                    </div>
                  </v-col>
                </v-row>
            ...
            <v-card id="roll" class="crd-trn ss-cards overflow-hidden mb-md-6 py-6" width="100%">
              <v-toolbar class="elevation-0 px-md-2 px-0 crd-trn" density="compact">
                ...
                <v-spacer></v-spacer>
                <v-select
                  @update:model-value="getRollingData()"
                  hide-details
                ...
                <v-menu v-model="menu2" :close-on-content-click="false" transition="scale-transition" offset-y min-width="auto">
                  <template v-slot:activator="{props}">
                    <v-text-field density="compact" class="rounded-pill" variant="solo" flat bg-color="secbg" v-model="rolldate" hide-details readonly append-icon="mdi-chevron-down" v-bind="props"></v-text-field>
                  </template>
                  <v-date-picker no-title :max="new Date().toISOString().split('T')[0]" class="rounded-xl" v-model="rolldate" @update:model-value="(menu2 = false), getRollingData()"></v-date-picker>
                </v-menu>
              </v-toolbar>
            ...
            <v-card id="peers" class="crd-trn ss-cards overflow-hidden mb-md-6 pt-6" width="100%">
              <v-toolbar class="elevation-0 px-md-2 px-0 crd-trn" density="compact">
                <v-list-item class="px-0">
                  ...
                  <v-list-item-subtitle class="maintext--text font-weight-medium fs-12">Comparison breakdown of {{ menudata ? menudata.fundName : "" }} information</v-list-item-subtitle>
                </v-list-item>
                <v-spacer></v-spacer>
                <v-select
                  @update:model-value="setPeersdata()"
                  hide-details
                  append-icon="mdi-chevron-down"
                  v-model="peerstab"
                ...
                <v-text-field v-model="peersearch" hide-details prepend-inner-icon="mdi-magnify" label="Search" class="rounded-pill d-none d-md-flex" density="compact" variant="solo" flat bg-color="secbg"></v-text-field>
              </v-toolbar>
              <v-data-table mobile must-sort :sort-by="[{ key: '10Year', order: 'desc' }]" hide-default-footer fixed-header class="rounded-0 overflow-y-auto mt-4" :headers="peerheader" height="334px" width="800px" :search="peersearch" :items="peeritem" :items-per-page="peeritem.length">
                ...
                <template v-slot:no-data>
                  <td colspan="100%" class="text-center pa-16">
                    <div class="mx-auto">
...
export default {
  ...
  computed: {
    peerheader() {
      return [
        ...
        {title: "AUM (Cr)", key: "aum", sortable: false, align: "end", class: "d-none d-md-table-cell"},
        {title: "10 Year %", key: "10Year", sortable: false, align: "end", class: "ws-p d-none d-md-table-cell"},
        {title: "Rating", key: "fundRat", sortable: false, align: "end", class: "ws-p d-none d-md-table-cell"},
      ];
    },
    mfholdheader() {
      return [
  ...
  methods: {
    ...
    putMForder(type) {
      eventBus.$emit("menudialog", "mforder", type, this.menudata.params);
    },
  },
};
</script>
