<template>
    ...
    <v-row no-gutters>
      <v-col cols="12">
        <p class="text-h6 font-weight-bold mb-3">Mutual funds NFO</p>
      ...
      <v-col cols="12">
        <v-card style="border: 1px solid var(--outline)" variant="outlined" class="elevation-0 rounded-lg mb-8 mb-md-0 crd-trn">
          <v-toolbar class="tool-sty elevation-0 py-4 mb-8 px-4 crd-trn" density="compact">
            <v-list-item class="pl-0">
              <template v-slot:prepend>
                <v-avatar class="mr-4">
                  <img width="80%" src="@/assets/mf/side-one.svg" />
                </v-avatar>
              </template>
              <v-list-item-title class="font-weight-bold text-h6">
                <v-badge color="primary" :content="mftabledata ? mftabledata.length : 0" :model-value="mftabledata && mftabledata.length > 0"><span>Ongoing new fund offerings</span></v-badge>
              </v-list-item-title>
              <v-list-item-subtitle class="fs-12 txt-666">A new fund offer (NFO) is the first subscription for any new fund by an investment company.</v-list-item-subtitle>
            </v-list-item>
            <v-spacer></v-spacer>
            ...
            <v-btn @click="mobilefilter = true" icon="true" class="px-0">
              <v-badge :model-value="mfcategorie.cat || mfcategorie.sub || mfcategorie.amc" dot>
            ...
            <v-text-field style="max-width: 160px" v-model="opensearch" hide-details prepend-inner-icon="mdi-magnify" label="Search" class="rounded-pill ml-1 d-none d-md-flex" density="compact" variant="solo" flat bg-color="secbg"></v-text-field>
          </v-toolbar>
          <v-data-table
            must-sort
              ...
              <table style="width: 100%; border-collapse: collapse;">
                ...
                <tbody v-if="mftabledata && mftabledata.length > 0">
                  <tr class="table-row" v-for="(item, o) in items" :key="o"
                    ...
                    <td class="pl-md-6 pl-4" style="width: 40%; vertical-align: middle; padding: 16px;">
                      <div class="d-flex align-center justify-space-between">
                        <div class="flex-grow-1" style="min-width: 0;">
                          <P @click="setSinglepage(item)" class="fs-13 font-weight-medium mb-1 primary--text table-hov-text"
                          ...
                          <v-chip-group column class="mb-0">
                            ...
                            <v-chip v-if="item.splitr" color="#F5F5F5" size="x-small" class="mr-1 mb-1" text-color="#666"
                              style="border-radius: 4px; padding: 4px 8px; height: 20px; font-size: 10px; display: inline-block;">
                              <span class="font-weight-medium">
                    ...
                    <td class="font-weight-medium pl-md-6 pl-4" style="width: 20%; vertical-align: middle; padding: 16px; text-align: left;">
                      {{ item.Start_Date ? item.Start_Date : "-" }}
                    </td>
                    <td class="font-weight-medium pl-md-6 pl-4" style="width: 20%; vertical-align: middle; padding: 16px; text-align: left;">
                      {{ item.End_Date ? item.End_Date : "-" }}
                    </td>
                    <td class="text-right font-weight-medium pr-md-6 pr-4" style="width: 20%; vertical-align: middle; padding: 16px; text-align: right;">
                      <span class="font-weight-medium maintext--text">₹{{ item.Minimum_Purchase_Amount ? Number(item.Minimum_Purchase_Amount).toFixed(2) : "0.00" }}</span>
                    </td>
                  </tr>
                  <tr v-if="showtable < mftabledata.length">
                    <td :colspan="tradeheader.length" class="text-center">
                      <v-btn v-if="!opensearch" color="primary" variant="text" class="elevation-0 text-none font-weight-bold py-4" @click="showtable += 24" block>show more</v-btn>
                    </td>
                  </tr>
                </tbody>
                <tbody v-else>
      ...
      <v-col cols="3" class="pt-2 pr-0 d-none d-md-none">
        <v-toolbar @click="addscript = true" flat density="compact" class="tool-sty">
          <p class="mb-0 font-weight-bold">
            Filters
            <v-chip class="ml-1" v-if="mfcategorie.cat || mfcategorie.sub || mfcategorie.amc" size="x-small" color="secbg" text-color="subtext" style="border-radius: 5px; padding: 10px 8px !important">
        ...
        <v-expansion-panels v-model="panel" class="expan">
            ...
            <v-expansion-panel-text class="px-0">
              <div>
                <div v-for="(w, j, l) in mfcategorie.cats" :key="l">
                  <v-list-item @click="mfcategorie.cat != j ? (mfcategorie.cat = j) : (mfcategorie.cat = ''), setChangewl()" class="crd-trn px-2">
          ...
          <v-expansion-panel>
            ...
            <v-expansion-panel-text class="px-0">
              <v-range-slider @update:model-value="setChangewl()" color="#FF1717" v-model="mfcategorie.minamt" :max="mfcategorie.minamt_max" :min="mfcategorie.minamt_min" hide-details :step="mfcategorie.minamt_max / 50" class="align-center mt-4">
                <template v-slot:prepend>
    ...
    <v-bottom-sheet v-model="mobilefilter" class="d-md-none">
      <v-sheet class="rounded-t-xl px-4" height="70vh">
        ...
        <v-chip-group v-if="mfcategorie.cat != '' || mfcategorie.sub != '' || mfcategorie.amc != ''" column class="my-0">
          <v-chip class="rounded-lg" v-if="mfcategorie.cat" color="secbg" text-color="subtext" size="x-small" style="border-radius: 5px; padding: 10px 8px !important">
            {{ mfcategorie.cat.replace("Scheme", "") }} <v-icon @click="(mfcategorie.cat = ''), setChangewl()" size="14" color="#666" class="ml-2"> mdi-close </v-icon>
          </v-chip>
          <v-chip class="rounded-lg" v-if="mfcategorie.sub" color="secbg" text-color="subtext" size="x-small" style="border-radius: 5px; padding: 10px 8px !important">
            {{ mfcategorie.sub }} <v-icon @click="(mfcategorie.sub = ''), setChangewl()" size="14" color="#666" class="ml-2"> mdi-close </v-icon>
          </v-chip>
          <v-chip class="rounded-lg" v-if="mfcategorie.amc" color="secbg" text-color="subtext" size="x-small" style="border-radius: 5px; padding: 10px 8px !important">
            {{ mfcategorie.amc.split("_")[0] }} <v-icon @click="(mfcategorie.amc = ''), setChangewl()" size="14" color="#666" class="ml-2"> mdi-close </v-icon>
          </v-chip>

          <v-chip class="rounded-lg" color="secbg" text-color="subtext" size="x-small" style="border-radius: 5px; padding: 10px 8px !important">
