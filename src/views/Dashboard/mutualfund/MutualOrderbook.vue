    ...
    <div>
        ...
        <div v-if="tab==='sip'">
            <v-toolbar flat dense class="tool-sty pl-4 crd-trn">
                ...
                <v-spacer></v-spacer>
                <v-text-field style="max-width: 220px" v-model="opensearchsip" hide-details prepend-inner-icon="mdi-magnify"
        ...
        <div v-else-if="tab==='orders'">
            ...
            <v-data-table :headers="orderheader" :items="filteredOrders" fixed-header :hide-default-footer="true" :loading="loading"
                ...
                <template #item.status="{ item }">
                    <v-tooltip location="bottom" color="black">
                        <template #activator="{ props }">
                            <p v-bind="props" class="ws-p font-weight-medium maintext--text mb-0">{{ item.status || '' }}</p>
                        </template>
                ...
                <template #item.action="{ item }">
                    <div class="d-inline-flex" @click.stop>
                        <v-btn v-if="item.status==='PAYMENT NOT INITIATED' && item.Remarks!=='ALLOTMENT DONE'" size="small" variant="tonal" color="secred" @click="openCancelOrder(item)">
                            Cancel
                        </v-btn>
                    </div>
                </template>
        ...
        <v-dialog v-model="canceldialog" max-width="420">
            <v-card class="rounded-xl elevation-0 text-center pt-8 pb-6 overflow-hidden">
                ...
                <div class="px-6 mb-6" v-if="dialogHead==='Pause SIP Order'">
                    <v-select v-model="sipInstalments" hide-details append-icon="mdi-chevron-down" class="rounded-pill mb-2" :items="[...Array(30).keys()].map(n => (n+1).toString())" variant="solo" :bg-color="'secbg'" density="comfortable" placeholder="Select No Of Instalment Amount" />
                </div>
                <v-row class="px-6">
                    <v-col cols="6">
                        <v-btn @click="canceldialog=false; singledata=null; dialogHead=''" color="outline" class="rounded-pill text-none subtext--text font-weight-bold elevation-0" block height="40">No</v-btn>
                    </v-col>
                    <v-col cols="6">
                        <v-btn :loading="loading" :disabled="dialogHead==='Pause SIP Order' && !sipInstalments" @click="confirmAction" color="btnclr" class="rounded-pill text-none btntext--text font-weight-bold elevation-0" block height="40">Yes</v-btn>
                    </v-col>
                </v-row>
            </v-card>
        </v-dialog>
