      ...
      <v-row no-gutters>
        <v-col cols="12">
          <v-card style="border: 1px solid var(--outline);" class="crd-trn elevation-0 rounded-lg mb-8 mb-md-0">
            <v-toolbar class="tool-sty elevation-0 pt-4 pb-0 mb-0 px-1 crd-trn" density="compact">
              <v-row align="center" class="w-100">
    <!-- First column: Image -->
    <v-col cols="auto">
      <v-img :src="mainCard4Src" width="50" height="50" />
    </v-col>
  
    <!-- Second column: Title & Subtitle -->
    <v-col>
      <div>
        <div class="font-weight-bold title">IPO Performance {{ year }}</div>
        <div class="fs-12 subtext--text">
          Monitor the IPO Performance of Mainline <br class="d-md-none"> IPO Stocks listed at BSE and NSE
              ...
              <v-text-field :disabled="loading" style="width:200px;" v-model="year" hide-details
                prepend-inner-icon=""  class="rounded-pill mr-4 d-none d-md-flex" density="compact" variant="solo" flat
                bg-color="secbg">
                <template #append>
                  <div class="d-flex flex-column">
                    <v-icon size="20" @click="setYearupdate('+')">mdi-chevron-up</v-icon>
                    <v-icon size="20" @click="setYearupdate('-')">mdi-chevron-down</v-icon>
                  </div>
                </template>
              </v-text-field>
              ...
              <v-text-field style="max-width:220px;" v-model="opensearch" hide-details
                ...
                class="rounded-pill mr-4  d-none d-md-flex" density="compact" variant="solo" flat
            ...
            <v-toolbar class="tool-sty elevation-0 py-0 mb-2 px-0 d-md-none" color="#fff" density="compact">
              <v-text-field style="max-width:100px;" :disabled="loading" v-model="year" hide-details
                prepend-inner-icon="" label="Year" class="rounded-pill mr-4" density="compact" variant="solo" flat
                bg-color="secbg">
                <template #append>
                  <div class="d-flex flex-column">
                    <v-icon size="20" @click="setYearupdate('+')">mdi-chevron-up</v-icon>
              ...
              <v-text-field v-model="opensearch" hide-details prepend-inner-icon="mdi-magnify"
                label="Search bonds" class="rounded-pill" density="compact" variant="solo" flat
                bg-color="secbg"></v-text-field>
            </v-toolbar>
  
            <v-data-table :sort-by="[{ key: 'dateis', order: 'desc' }]" :search="opensearch" must-sort
            ...
            <div class="d-md-none">
              <div v-if="iposdatas.length > 0">
                <v-card class="rounded-lg mb-3" variant="outlined" v-for="(p, t) in iposdatas.length < showtable ? iposdatas.slice(0, showtable) : iposdatas"
                  :key="t">
                  <div>
                ...
                <v-btn v-if="showtable < iposdatas.length" color="primary" variant="text" class="elevation-0 text-none font-weight-bold py-4"
