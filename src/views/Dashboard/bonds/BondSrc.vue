    ...
    <v-row no-gutters>
      <v-col cols="12">
        ...
        <v-chip-group v-model="bondtype" class="d-md-none">
          <v-chip 
            v-for="(t, b) in bondsitem" 
            :key="b"
            :color="bondtype == b ? '#000' : 'subtext666'" 
            :text-color="bondtype == b ? '#fff' : 'subtext666'" 
            :variant="bondtype != b ? 'outlined' : 'flat'"
            @click="bondtype = b"
          >
            {{ t }}
          </v-chip>
        ...
        <div class="default-tab">
          <v-tabs v-model="bondtype" color="primary" class="d-none d-md-flex" fixed-tabs>
            <v-tab 
              v-for="(t, b) in bondsitem" 
              :key="b"
              class="font-weight-bold subtitle-2 mb-0 text-none"
            >
              <v-badge 
                :content="(bondsdatas[b] && bondsdatas[b].length) ? bondsdatas[b].length : ''" 
        ...
        <v-data-table
          ...
          :items="currentBondsData"
          :items-per-page="currentBondsData.length > 0 ? showtable : 0"
        >
          <template v-slot:body="{ items, headers }">
            <tbody v-if="currentBondsData.length > 0">
              <tr 
                class="table-row" 
                v-for="(item, o) in items" 
                :key="o"
              ...
              >
                <td class="pos-rlt" style="width: 36%; vertical-align: middle; padding: 16px;">
                  <div class="d-flex align-center justify-space-between">
                    <div class="flex-grow-1" style="min-width: 0;">
                      <P class="fs-13 font-weight-medium mb-1 txt-000 table-hov-text">
                        {{ item.name ? item.name : "" }}
                      </P>
                      <v-chip-group column class="mb-0">
                        <v-chip 
                          color="secbg" 
                          size="x-small" 
                          class="table-hov-prd mr-1 mb-1" 
                          text-color="subtext" 
                          style="border-radius: 5px; padding: 4px 8px !important; height: 20px; font-size: 10px; display: inline-block;"
                        >
                    ...
                    >
                      <v-btn 
                        @click="uid ? bondOrder(item, bondtype) : $emit('gologin')" 
                        size="small" 
                ...
                <td style="vertical-align: middle; padding: 16px;">
                  <v-list-item style="width: 220px" class="px-0">
                    <v-list-item-content>
                      <v-list-item-subtitle class="font-weight-medium txt-666 fs-10 mb-1">Start date</v-list-item-subtitle>
                      <v-list-item-title class="font-weight-bold caption mb-1">{{ item.biddingStartDate || '' }}</v-list-item-title>
                      <v-list-item-subtitle class="font-weight-medium fs-10">{{ item.dailyStartTime || '' }}</v-list-item-subtitle>
                    </v-list-item-content>
                    <v-list-item-content class="text-right">
                      <v-list-item-subtitle class="font-weight-medium txt-666 fs-10 mb-1">End date</v-list-item-subtitle>
                      <v-list-item-title class="font-weight-bold caption mb-1">
                ...
                <td class="text-right font-weight-medium" style="vertical-align: middle; padding: 16px;">
                  ₹{{ item.lotSize ? Number(item.lotSize).toLocaleString() : "0.00" }} <br />
                  <span class="subtext--text font-weight-regular fs-12">{{ item.lotSize ? item.lotSize / 100 : "0" }} Qty.</span>
                </td>
                <td class="text-right font-weight-medium" style="vertical-align: middle; padding: 16px;">
                  {{ item.issueSize ? (item.issueSize / 10000000).toFixed(2) : "0.00" }} 
                  <span class="subtext--text font-weight-regular fs-12">Cr.</span>
                </td>
              </tr>
              <tr v-if="showtable < currentBondsData.length">
                <td :colspan="headers.length">
                  <v-btn 
                    color="primary" 
                    variant="text" 
                    class="elevation-0 text-none font-weight-bold py-4" 
                    @click="showtable += 24" 
                    block
                  >
                    show more
              ...
              <td :colspan="headers.length" class="text-center">
                <div class="mx-auto py-16 mt-16">
                  ...
                  <h4 class="txt-999 font-weight-regular caption">
                    There is no {{ getBondTypeName(bondtype) }} data here yet!
                  </h4>
                </div>
              </td>
            </tbody>
        ...
        <div class="d-md-none overflow-hidden">
          ...
          <div v-if="currentBondsData.length > 0">
            <v-card 
              ...
              :key="o" 
              class="elevation-0 rounded-0 overflow-hidden"
            >
              <div class="px-0">
                    ...
                    <v-chip-group column class="mb-1">
                      ...
                      <v-chip class="px-0 crd-trn" size="x-small" text-color="#000">
                        {{
                          ...
                          ${item.biddingEndDate ? formatDate(item.biddingEndDate) : ''} ${item.dailyEndTime ? item.dailyEndTime.slice(0, 5) : ''}`
                        }}
                      </v-chip>
                    </v-chip-group>
                  </v-list-item-content>
                ...
                <v-row class="py-2" no-gutters>
                  <v-col cols="6">
                    ...
                    <p class="mb-1 fs-12 txt-666">
                      Min. Inv: <span class="font-weight-bold black--text">₹{{ item.lotSize ? Number(item.lotSize).toLocaleString() : "0.00" }}</span>
                    </p>
                  </v-col>
              ...
              <div class="px-4">
                <v-btn 
                  ...
                  @click="uid ? bondOrder(item, bondtype) : $emit('gologin')" 
                  size="small" 
                  class="elevation-0 rounded-pill text-none primary--text font-weight-bold mb-2" 
                  color="secbg"
                >
                  Invest
                </v-btn>
          ...
          <v-btn 
            v-if="showtable < currentBondsData.length" 
            color="primary" 
            variant="text" 
            class="elevation-0 text-none font-weight-bold py-4 mb-1" 
            @click="showtable += 24" 
            block
          >
            show more
