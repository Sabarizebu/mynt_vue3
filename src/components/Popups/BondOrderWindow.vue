<template>
  <div>
    <v-dialog v-model="bondorderdialog" persistent @click:outside="!orderpoploader ? closeMenudialog('bondorder') : ''"
      max-width="400">
      <v-card class="pb-1 overflow-hidden" color="cardbg" rounded="xl">
        <v-card class="elevation-0 pt-2 pb-1" color="secbg">
          <v-toolbar class="elevation-0 px-1 px-md-2 crd-trn" density="compact">
            <P class="font-weight-bold fs-16 maintext--text mb-0 px-4">
              Order {{ menudata.type == 0 ? "G-SEC" : menudata.type == 1 ? "T-BILL" : menudata.type == 2 ? "SDL" :
                menudata.type == 3 ? "SGB" : "" }}
            </P>
            <v-spacer></v-spacer>
            <v-btn :disabled="orderpoploader" @click="closeMenudialog('bondorder')" color="maintext" size="x-small"
