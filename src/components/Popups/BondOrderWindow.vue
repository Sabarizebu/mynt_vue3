<template>
  <div>
    <v-dialog 
      v-model="bondorderdialog" 
      persistent
      @click:outside="!orderpoploader ? closeMenudialog('bondorder') : ''" 
      max-width="400"
    >
      <v-card class="pb-5 overflow-hidden" color="cardbg">
        <v-card class="elevation-0 pt-2 pb-1" color="secbg">
          <v-toolbar class="elevation-0 px-1 px-md-2 crd-trn" density="compact">
            <P class="font-weight-bold fs-16 maintext--text mb-0">
              Order {{ menudata.type == 0 ? "G-SEC" : menudata.type == 1 ? "T-BILL" : menudata.type == 2 ? "SDL" : menudata.type == 3 ? "SGB" : "" }}
            </P>
            <v-spacer></v-spacer>
            <v-btn 
              :disabled="orderpoploader" 
              @click="closeMenudialog('bondorder')" 
              color="maintext" 
              size="x-small"
              variant="outlined"
              icon
        ...
        <div class="px-4 px-md-6 pt-4 pb-2">
          ...
          <v-text-field 
            :model-value="bondqty"
            @update:model-value="bondqty = $event"
            @keyup="handleQtyKeyup"
            density="comfortable"
            variant="solo"
            flat
            :bg-color="'secbg'"
          ...
          <v-card 
            v-if="menudata.ledger && menudata.cutoffPrice && menudata.ledger < menudata.cutoffPrice * bondqty" 
            color="primhover"
          ...
          >
            <p class="mb-0 primary--text fs-12">
              ...
              <v-btn 
                @click="closeMenudialog('bondorder')" 
                variant="text" 
                class="text-none font-weight-black px-0"
                size="small" 
                color="primary" 
                to="/funds"
              >
        ...
        <v-toolbar class="tool-sty elevation-0 pt-4 mb-2 px-4 px-md-6 crd-trn" density="compact">
          <span class="font-weight-regular fs-12 subtext--text">
            Price : <span class="primary--text font-weight-bold"> 
          ...
          <v-btn 
            :disabled="!bondqty || bondqty < menudata.minbidqty || (menudata.ledger && menudata.cutoffPrice && menudata.ledger < menudata.cutoffPrice * bondqty)"
            @click="setBondorder()" 
            ...
            height="40"
          >
            Invest ₹{{ menudata.cutoffPrice && bondqty ? (menudata.cutoffPrice * bondqty).toFixed(2) : "0.00" }}
          </v-btn>
        </v-toolbar>
      </v-card>
    </v-dialog>
