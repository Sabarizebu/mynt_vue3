    ...
    <v-dialog v-if="menudata.flow >= 0 || menudata.flow == 'mod'" v-model="iposorderdialog" persistent
      ...
      <v-card class="pb-4 pt-2  overflow-hidden" color="cardbg" style="border-radius: 15px;">
        <v-toolbar class="elevation-0 px-1 px-md-2 crd-trn" density="compact">
          <p class="font-weight-bold  maintext--text mb-0 pl-4">IPO Application</p>
          <v-spacer></v-spacer>
          <v-btn :disabled="orderpoploader" @click="closeMenudialog('iposorder')" color="maintext" size="x-small"
            variant="outlined" icon class="mr-3">
        ...
        <v-card class="elevation-0 rounded-t-lg rounded-b-0 pt-3 mx-1 mx-md-6 px-3" color="secbg">
          <p class=" font-weight-bold mb-0 fs-13 table-hov-text mb-0">
            {{ menudata.name ? menudata.name : "" }}
            <v-chip size="x-small" variant="flat" class="ml-2"
              style="border-radius: 5px; background-color: #ffffff !important; color: #666666 !important;">
              <span class=" fs-10">
                {{ menudata.symbol }}
              </span>
            </v-chip>
            <v-chip v-if="menudata.flow >= 0" size="x-small" variant="flat" class="ml-2"
              style="border-radius: 5px; background-color: #ffffff !important; color: #666666 !important;">
              <span class=" fs-10">
          ...
          <v-row class="px-0 py-3 ">
            <v-col cols="4">
              <div>
                <div class="font-weight-medium fs-12 maintext--text mb-0">{{
        ...
        <v-card class="elevation-0 rounded-b-lg rounded-t-0 py-2 mx-1 mx-md-6 px-4" color="#FCEFD4">
          <p class="grey--text fs-12 mb-0">
        ...
        <div v-if="iposbids.length > 0" id="bodytab" class="px-4 px-md-6 pt-4 pb-2 overflow-y-auto"
            ...
            <div v-if="b.bitis">
              <p class="maintext--text fs-12 fs-12 font-weight-medium mb-2">
                ...
                <v-btn
                  ...
                  @click="iposbids[i].bitis = false" color="mainred" class="text-none font-weight-bold" variant="text"
              ...
              <v-row no-gutters>
                <v-col cols="9">
                  <v-row no-gutters>
                    <v-col cols="6" class="pr-3">
                      ...
                      <v-text-field :disabled="orderpoploader" density="compact" bg-color="secbg" flat rounded="pill"
                        ...
                        variant="solo" type="number" hide-spin-buttons :min="menudata.minBidQuantity" hide-details
                        :step="menudata.lotSize" :max="menudata.maxbidqty" v-model="iposbids[i].qty">
                        <template #append-inner>
                          <v-btn :disabled="orderpoploader"
                            @click="iposbids[i].qty = Number(iposbids[i].qty) + Number(menudata.lotSize); ipoCalcs()"
                            icon class="elevation-0" style="width: 28px; height: 28px; min-width: 28px;">
                        ...
                        <template #prepend-inner>
                          <v-btn :disabled="iposbids[i].qty <= menudata.lotSize || orderpoploader"
                    ...
                    <v-col cols="6" class="pl-3">
                      ...
                      <v-text-field density="compact" height="40px" bg-color="secbg" flat rounded="pill" variant="solo"
                        type="number" hide-details hide-spin-buttons :disabled="iposbids[i].cutoff || orderpoploader"
                        ...
                        :min="menudata.minPrice" :max="menudata.maxPrice" v-model="iposbids[i].price">
                        <template #prepend-inner>
                          <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
                            fill="none">
                            <path
                ...
                <v-col cols="3" class="mt-auto pt-5">
          ...
          <v-btn v-if="menudata.flow >= 0 && !iposbids[2].bitis" :disabled="orderpoploader"
            ...
            variant="text" color="primary" class="text-none mt-0">+ Add another bid </v-btn>
          ...
          <v-form :class="menudata.flow >= 0 ? '' : 'pt-3'" ref="iposupiformRef" v-model="iposupivalid"
            ...
            <p class="font-weight-medium fs-14 maintext--text mb-2" :class="!iposbids[2].bitis ? 'mt-2' : 'mt-6'">UPI ID
              (Virtual payment address)</p>
            <v-text-field :disabled="orderpoploader" required height="40px" density="compact" bg-color="secbg" flat
              rounded="pill" variant="solo" v-model="iposupiid" @keyup="upiidfield = false"
              :rules="[(v) => !!v || 'Upi Id is required', (v) => /.+@.+/.test(v) || 'Upi Id must be valid']"
              placeholder="Add UPI ID" :error-messages="upiIDerrors">
            </v-text-field>
            <v-checkbox id="check"
              :disabled="menudata.iposmax || menudata.iposrange || menudata.iposminqty || orderpoploader" required
              :rules="[(v) => !!v || 'You must agree to invest!']" color="maintext" v-model="iposcheckbox" class="mt-0"
              hide-details>
              <template #label>
        ...
        <v-toolbar class="tool-sty elevation-0 pt-3 mb-2 px-4 px-md-6 crd-trn" density="compact">
          <span class="font-weight-regular fs-12 subtext--text">
            Total investment : <span class="font-weight-bold" style="color: #0037B7;"> ₹{{ menudata.ipostotal ?
              menudata.ipostotal.toFixed(2) : "" }}</span>
          </span>
          <v-spacer></v-spacer>
          <!-- <v-btn @click="closeMenudialog('iposorder')" class="text-none rounded-pill elevation-0 px-6" height="40"
            color="#f4f5f9" variant="flat">
            <span style="color: #555; font-weight: 500;">Cancel</span>
          </v-btn> -->
          <v-btn @click="setUpiValid()"
            :disabled="!iposupivalid || menudata.iposmax || menudata.iposrange || menudata.iposminqty || upiidfield"
            type="submit" :loading="orderpoploader" text-color="white"
            class="blk text-none rounded-pill elevation-0  px-6 ml-4" height="40px">Continue
          </v-btn>
        </v-toolbar>
      </v-card>
    </v-dialog>
    ...
    if (itemdata.subCategorySettings) {
      ...
      for (var q = 0; q < itemdata.subCategorySettings.length; q++) {
        if (itemdata.subCategorySettings[q].allowUpi) {
          const subCatCode = itemdata.subCategorySettings[q].subCatCode
          if (subCatCode == "IND" && !addedKeys.has("IND")) {
            menudata.value.categorys.push({ key: "IND", val: "Individual", data: itemdata.subCategorySettings[q] })
            addedKeys.add("IND")
          } else if (subCatCode == "EMP" && !addedKeys.has("EMP")) {
            menudata.value.categorys.push({ key: "EMP", val: "Employee", data: itemdata.subCategorySettings[q] })
            addedKeys.add("EMP")
          } else if (subCatCode == "SHA" && !addedKeys.has("SHA")) {
            menudata.value.categorys.push({ key: "SHA", val: "Shareholder", data: itemdata.subCategorySettings[q] })
            addedKeys.add("SHA")
          } else if (subCatCode == "POL" && !addedKeys.has("POL")) {
            menudata.value.categorys.push({ key: "POL", val: "Policyholder", data: itemdata.subCategorySettings[q] })
            addedKeys.add("POL")
          }
        }
      }
      // Remove duplicates based on key property
      const uniqueCategories = []
      const seenKeys = new Set()
      for (let i = 0; i < menudata.value.categorys.length; i++) {
        if (!seenKeys.has(menudata.value.categorys[i].key)) {
          uniqueCategories.push(menudata.value.categorys[i])
          seenKeys.add(menudata.value.categorys[i].key)
        }
      }
      menudata.value.categorys = uniqueCategories
      if (menudata.value.categorys.length > 0) {
